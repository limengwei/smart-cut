...
- **前置**: Plan 1-5 已完成（脚手架/Adapters/Pipeline-Service/API-Frontend/Timeline），主线 transcribe→analyze→edit→export 已跑通；`SubtitleStep` 当前为 `skipped (MVP)` 占位，`frontend/src/remotion/` 仅有 `.gitkeep`。
- **2026-07-03 修订**: 高亮粒度从"逐字"降级为"整句"。原方案依赖 whisper 词级时间戳，但词级链路当前未打通（`parseWhisperJSON` 不解析 `-owts`，`Transcript.Words` 从不填充）。整句高亮直接用现有 `Segment.StartMs/EndMs`，零前置依赖，范围更聚焦。逐字高亮留作未来扩展（需先打通词级解析）。
 ## 1. 目标

 实现 Smart-Cut 的最后一块 MVP 能力：**Remotion 动态字幕（逐字高亮）**，覆盖预览与导出两个场景，并复用现有 pipeline/eventbus 进度机制。
实现 Smart-Cut 的最后一块 MVP 能力：**Remotion 动态字幕（整句高亮）**，覆盖预览与导出两个场景，并复用现有 pipeline/eventbus 进度机制。
 ### 1.1 MVP 范围
 ...
 - "清理工程缓存"按钮
 - 字幕模板/片头片尾（非字幕功能）
- **逐字高亮**（词级时间戳驱动）：需先打通 whisper `-owts` 解析链路，作为独立未来任务

 ## 2. 关键技术决策

 | 决策点 | 选定方案 | 理由 |
 |---|---|---|
| 技术路线 | Player + Node 脚本 worker 导出 | 既支持预览整句高亮，又支持导出产物含字幕。MVP 不打包 worker，开发机装 Node 即可 |
 | 渲染时机 | 逐段渲染后拼接 | 渲染时长 = 各段时长之和（最短），且视频和字幕片段一一对应拼接逻辑清晰 |
| 高亮数据源 | whisper segment 时间戳 | 直接用现有 `Segment.StartMs/EndMs`，零前置依赖；当前词级链路未打通，整句高亮是 MVP 务实选择 |
 ...
 ```
 L1. UI Layer
    ├── RemotionPlayer.tsx         # <Player> 包装，叠加在 <video> 上，整句高亮
     └── remotion/SubtitleComp.tsx  # 字幕 Composition（纯 React 组件，前后端同构）
 L2. API Layer (App)
    └── GetSubtitleConfig          # 新增：返回当前 SubtitleStyle + Segments 给前端 Player
 L3. Service Layer
...
**时间基准约定**：`segments[].startMs` 一律是**相对于 keep 段起点**的偏移。调用方（Service 层）负责偏移计算，Composition 本身不感知绝对时间。

**整句高亮逻辑**：遍历 `segments`，找出当前时间命中的那个 segment（`timeMs >= seg.startMs && timeMs < seg.endMs`），用 highlight 色显示其文本；其余时段空帧。无需词级数据，直接用 segment 时间戳。
 ...
 type SubtitleSegmentRequest struct {
     ...
     EndMs        int64
    Segments     []model.Segment    // 落在本 keep 段内的字幕句段（已偏移为段内相对时间）
...
Worker 调用：`exec.CommandContext(ctx, nodePath, workerScriptPath)`，通过 stdin 传入 JSON（避免命令行长度限制，句段多时安全）。worker 解析后调 `@remotion/renderer.renderMedia`，输出 `<OutputDir>/subtitle_<SegmentID>.mp4`。`Width`/`Height`/`Fps` 取自 `project.Media`，保证字幕透明层与视频帧尺寸、帧率严格一致，overlay 时不错位。
 ...
 <div className="relative">
  ...
  {subtitleEnabled && transcript?.segments?.length && (
     <div className="absolute inset-0 pointer-events-none">
       <RemotionPlayer
         component={SubtitleComp}
        inputProps={{ segments: transcript.segments, style: subtitleStyle }}
...
- 整句高亮纯靠 seek 响应 playheadMs，不让 Player 自己跑时间线，避免双时钟漂移。
- 刷新频率取决于 video 的 `timeupdate` 事件（约 4-66Hz），对整句高亮够用（句级切换频率远低于词级）。
...
export const SubtitleComp: React.FC<{ segments: { id: number; text: string; startMs: number; endMs: number }[]; style: SubtitleStyle }> = ({
  segments, style,
 }) => {
  ...
  if (!active) return <AbsoluteFill />;  // 无命中，空帧

  // 整句高亮：当前命中句用 highlight 色，按 style.position 定位
  return (
    <AbsoluteFill>
      <div style={{ position: "absolute", ...positionStyle(style.position), ... }}>
        {active.text}
      </div>
    </AbsoluteFill>
  );
...
整句高亮：当前命中的 segment 全文用 highlight 色显示。无词级拆分，无需 WordIDs。
...
| 转录句段缺失 | `transcript.Segments` 为空（未转录） | SubtitleService 直接跳过字幕，正常无字幕导出 |

 ### 6.2 关键回退策略：字幕失败不阻断导出

 字幕是"锦上添花"，不应因字幕问题导致整个导出失败。**分层降级**：

 1. `transcript.Segments` 为空 → SubtitleService 不启动 SubtitleStep，ExportStep 走原逻辑（无字幕）
 ...
 **清理规则**（与主设计文档 8.3 一致）：
 ...
 ```
stdin  → 单行 JSON: { segmentID, startMs, endMs, segments, style, outputPath, width, height, fps }
...
- `SubtitleComp` active 句段判定与高亮渲染

 ## 8. 与现有代码的衔接点

- **`internal/pipeline/steps.go`**：现有 `SubtitleStep` 占位需重写。注：`NewTranscribeStep` 的 unreachable code bug 已在 P1 导出管线改造中修复（commit 0a2a4a3），无需再处理。
...
- **`internal/adapter/ffmpeg.go`**：P1 改造后接口为 `ExtractSegment` + `ConcatDemuxer`（lossless）/ `ConcatReencode`（reencode）。字幕 overlay 需在提取或拼接阶段注入字幕片段（实现时定具体方法）。
...
- **P1 改造后的衔接**：ExportStep 现已按 `ExportMode` 分支（lossless 走 ExtractSegment+ConcatDemuxer，reencode 走 ConcatReencode）。字幕 overlay 需与这两条路径协同，实现时需决定 overlay 注入点（提取阶段内嵌 vs 拼接阶段外挂）。
...
- **2026-07-03 初版**：逐字高亮（whisper 词级时间戳驱动）
- **2026-07-03 修订**：降级为整句高亮（segment 时间戳驱动）。原因：词级时间戳链路当前未打通（`parseWhisperJSON` 不解析 `-owts`，`Transcript.Words` 从不填充，`TranscribeService` 默认 `WordLevel: false`）。整句高亮直接用现有 `Segment.StartMs/EndMs`，零前置依赖。逐字高亮留作未来扩展。
